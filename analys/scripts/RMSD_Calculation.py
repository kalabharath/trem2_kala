...
def get_rmfs_coordinates(path, idfile_A, idfile_B, subunit_name):
    ...
    models_name = []
    # iterate over the sample name and sample id lists in parallel, returns A f1, B f2
    for sample_name,sample_id_file in zip(['A','B'],[f1,f2]):
        # for each file in the list of samples and identifiers_A, sort the file by the # of the .rmf file
        for str_file in sorted(glob.glob("%s/sample_%s/*.rmf3" % (path,sample_name)),key=lambda x:str(x.split('/')[-1].split('.')[0])):
